...
class MyApplicationViewController: UIViewController {
    ...
    let viewModel: MyApplicationViewModel
    var jobs = [Job]()

    init(viewModel: MyApplicationViewModel) {
...
extension MyApplicationViewController: UITableViewDelegate, UITableViewDataSource, UITableViewDataSourcePrefetching {
    func tableView(_ tableView: UITableView, prefetchRowsAt indexPaths: [IndexPath]) {
        ...
        guard lastIndexPath.row >= viewModel.appliedJobsSubject.value.count - 1,
              currentPage < totalPages else { return }

        viewModel.currentPageSubject.send(currentPage + 1)
        viewModel.loadAppliedJobListing(page: viewModel.currentPageSubject.value)
    }
